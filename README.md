...
- **平移與歸位**：預設拖曳是旋轉，點工具列「平移」（或按 <kbd>P</kbd>）後拖曳即可平移畫面，手機單指平移、雙指縮放；按「歸位」（或 <kbd>R</kbd>）回到該章節的預設構圖。被面板或螢幕邊緣擋住的部位都能這樣移到看得到的位置。
- **部位解剖**：點選琴身各部位查看說明（含 F 孔、琴橋、肩墊），按 <kbd>E</kbd> 切換分解視圖（含音柱）。
...
- **面板可移動**：拖曳說明面板的「拖曳移動」把手就能把面板搬到任何位置，連拖出畫面也行，不會擋住 3D 小提琴；方向鍵微調（<kbd>Shift</kbd> 加速），<kbd>Home</kbd> 或按一下把手歸位。按把手旁的 ✕ 可收起面板，左上角會留下圖示，點一下就能復原。
- **鍵盤操作**：<kbd>←</kbd> <kbd>→</kbd> 翻頁、<kbd>E</kbd> 分解視圖、<kbd>P</kbd> 切換平移、<kbd>R</kbd> 視角歸位、<kbd>?</kbd> 開關說明、<kbd>Esc</kbd> 取消選取。
...
- 3D 場景需要 WebGL。若瀏覽器無法建立 WebGL 內容（例如停用硬體加速），網站會顯示明確說明，並保留全部文字內容、章節切換與小測驗。
